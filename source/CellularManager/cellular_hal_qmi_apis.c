...
typedef enum {
    ...
    MODEM_OPEN_STATE_GET_MANUFACTURER,
...
typedef enum {
    ...
    DMS_GET_MANUFACTURER_VIA_DEVICE_OPEN,
    DMS_GET_MANUFACTURER_VIA_DML
...
static int getProfileRetry = 0; //Retry if the modem profile list is empty
static int getProfileReset = 0; //Determine if a profile is already present to avoid unnecessary creation
static int getProfileRetryToCreate = 0; //Checks if the profile exists before attempting to create it
...
int
cellular_hal_qmi_init_ContextDefaultProfile
    (
        CellularContextInitInputStruct *pstCtxInputStruct
    )
{
...
int cellular_hal_qmi_get_cell_information(CellularCellInfo *pCell_info, unsigned int *pTotal_cell_count)
{
    if ( (pCell_info == NULL) || (pTotal_cell_count == NULL) ) {
        CELLULAR_HAL_DBG_PRINT("%s-%d: invalid QMI cell info input\n", __FUNCTION__, __LINE__);
        return RETURN_ERROR;
    }
    ...
    *pTotal_cell_count = 0;
        ...
        ( TRUE == qmi_device_is_open( gpstQMIContext->qmiDevice ) ) )
    {
        ...
        if( NULL != nasCtx->nasClient )
        {
            ...
            *pTotal_cell_count = nasCtx->iTotalNoofCellInfo;
            ...
            if ( nasCtx->iTotalNoofCellInfo > CELLULAR_QMI_INTER_FREQ_MAX_CNT ) {
                CELLULAR_HAL_DBG_PRINT("%s-%d: Error! total cell count (%u) greater than available memory\n", __FUNCTION__, __LINE__, *pTotal_cell_count);
                *pTotal_cell_count = CELLULAR_QMI_INTER_FREQ_MAX_CNT;
            }
    ...
    {
        CELLULAR_HAL_DBG_PRINT("%s - QMI(%s) not ready so can't be collect cell information\n", __FUNCTION__, gpstQMIContext->modem_device_name);
        return RETURN_ERROR;
    }
}
...
static void cellular_qmi_get_cell_location_info(QmiClientNas *nasClient,
                                                ...
                                                gpointer user_data)
{
    QmiMessageNasGetCellLocationInfoOutput *output = NULL;
    GError *error = NULL;

    guint16 absolute_rf_channel_number = 0;
    guint32 global_cell_id = 0;
    guint16 serving_cell_id = 0;
    guint16 tracking_area_code = 0;
    guint32 timing_advance = 0;
    guint   bandInfo = 0;

    GArray *pInterFreq = NULL;
    GArray *pIntraFreqCells = NULL;

    int intraNeighborCnt = 0;
    int interNeighborCnt = 0;
    int totalCellCnt = 0;
    int localCellCnt = 0;

    GTask *task = (GTask *)user_data;
    ContextDeviceOpen *pDeviceOpenCtx = g_task_get_task_data(task);
    QMIContextStructPrivate *pstQMIContext =
        (QMIContextStructPrivate *)pDeviceOpenCtx->vpPrivateData;
    ...
    CELLULAR_HAL_DBG_PRINT("%s: QMI cell location info retrieved successfully\n", __FUNCTION__);

    // Fetch Intra-frequency LTE info
    ...
    CELLULAR_HAL_DBG_PRINT("%s: IntraFreq raw cells=%u (includes serving cell)\n", __FUNCTION__, (pIntraFreqCells != NULL) ? pIntraFreqCells->len : 0);

    if (!qmi_message_nas_get_cell_location_info_output_get_lte_info_timing_advance(
            output, &timing_advance, &error)) {
        CELLULAR_HAL_DBG_PRINT("%s: failed to get timing advance\n", __FUNCTION__);
        g_clear_error(&error);
        goto CLEANUP;
    }
    ...
    nasCtx->trackingAreaCode = tracking_area_code;
    nasCtx->timingAdvance = timing_advance;
    nasCtx->rfcn = absolute_rf_channel_number;
    nasCtx->bandInfo = bandInfo;

    CELLULAR_HAL_DBG_PRINT("%s: ServingCell: GCI=%u PCI=%u TAC=%u RFCN=%u Band=%u TA=%u\n", __FUNCTION__,
        nasCtx->globalCellId, nasCtx->servingCellId, nasCtx->trackingAreaCode,
        nasCtx->rfcn, nasCtx->bandInfo, nasCtx->timingAdvance);

    // Exclude serving cell from intra neighbors
    if (pIntraFreqCells && pIntraFreqCells->len > 0)
        intraNeighborCnt = pIntraFreqCells->len - 1;

    CELLULAR_HAL_DBG_PRINT("%s: IntraFreq neighbors=%d (serving cell excluded)\n", __FUNCTION__, intraNeighborCnt);

    // Fetch Inter-frequency LTE info
    if (qmi_message_nas_get_cell_location_info_output_get_interfrequency_lte_info(
            output, NULL, &pInterFreq, &error)) {

        if (pInterFreq) {
            for (guint i = 0; i < pInterFreq->len; i++) {
                QmiMessageNasGetCellLocationInfoOutputInterfrequencyLteInfoFrequencyElement *freq =
                    &g_array_index(pInterFreq,
                                   QmiMessageNasGetCellLocationInfoOutputInterfrequencyLteInfoFrequencyElement,
                                   i);
                if (freq && freq->cell)
                    interNeighborCnt += freq->cell->len;
            }

	    CELLULAR_HAL_DBG_PRINT("%s: InterFreq EARFCN groups=%u, neighbor count=%u\n", __FUNCTION__, pInterFreq->len, interNeighborCnt);
        }
    } else {
        g_clear_error(&error);
    }
    ...
    totalCellCnt = intraNeighborCnt + interNeighborCnt;

    CELLULAR_HAL_DBG_PRINT("%s: Total neighbors calculated: intra=%d inter=%d total=%d\n", __FUNCTION__,
        intraNeighborCnt, interNeighborCnt, totalCellCnt);

    if (totalCellCnt <= 0)
        goto UPDATE_CTX;

    if (totalCellCnt > CELLULAR_QMI_INTER_FREQ_MAX_CNT)
        totalCellCnt = CELLULAR_QMI_INTER_FREQ_MAX_CNT;

    pCellInfo = calloc(totalCellCnt, sizeof(NASFrequencyLteCellInfo));
    if (!pCellInfo) {
        CELLULAR_HAL_DBG_PRINT("%s: memory allocation failed\n", __FUNCTION__);
        goto CLEANUP;
    }

    // Fill intra-frequency neighbors
    // NOTE: skip serving cell
    if (pIntraFreqCells) {
        for (guint i = 0; i < pIntraFreqCells->len && localCellCnt < totalCellCnt; i++) {
            ...
            if (cell->physical_cell_id == serving_cell_id)
                continue;

            pCellInfo[localCellCnt].physical_cell_id = cell->physical_cell_id;
            pCellInfo[localCellCnt].rsrq = cell->rsrq * 0.1;
            pCellInfo[localCellCnt].rsrp = cell->rsrp * 0.1;
            pCellInfo[localCellCnt].rssi = cell->rssi * 0.1;
            pCellInfo[localCellCnt].rfcn = absolute_rf_channel_number;

	    CELLULAR_HAL_DBG_PRINT("%s: IntraNeighbor[%d]: PCI=%u RSRP=%.1f RSRQ=%.1f RSSI=%.1f RFCN=%u\n", __FUNCTION__,
                localCellCnt, pCellInfo[localCellCnt].physical_cell_id, pCellInfo[localCellCnt].rsrp, 
		pCellInfo[localCellCnt].rsrq, pCellInfo[localCellCnt].rssi, pCellInfo[localCellCnt].rfcn);

	    // increment cell cnt index
            localCellCnt++;
        }
    }

    // Fill inter-frequency neighbors
    if (pInterFreq) {
        for (guint i = 0; i < pInterFreq->len && localCellCnt < totalCellCnt; i++) {
            QmiMessageNasGetCellLocationInfoOutputInterfrequencyLteInfoFrequencyElement *freq =
                &g_array_index(pInterFreq,
                               QmiMessageNasGetCellLocationInfoOutputInterfrequencyLteInfoFrequencyElement,
                               i);
            ...
            for (guint k = 0; k < freq->cell->len && localCellCnt < totalCellCnt; k++) {
                ...
                pCellInfo[localCellCnt].rsrp = cell->rsrp * 0.1;
                pCellInfo[localCellCnt].rssi = cell->rssi * 0.1;
                pCellInfo[localCellCnt].rfcn = freq->eutra_absolute_rf_channel_number;

		CELLULAR_HAL_DBG_PRINT("%s: InterNeighbor[%d]: RFCN=%u PCI=%u RSRP=%.1f RSRQ=%.1f RSSI=%.1f\n", __FUNCTION__,
                    localCellCnt, freq->eutra_absolute_rf_channel_number, pCellInfo[localCellCnt].physical_cell_id,
                    pCellInfo[localCellCnt].rsrp, pCellInfo[localCellCnt].rsrq, pCellInfo[localCellCnt].rssi);
...
UPDATE_CTX:
    ...
    nasCtx->iTotalNoofIntraFreqCellInfo = intraNeighborCnt; // cnt excluding serving cell
    nasCtx->iTotalNoofCellInfo = localCellCnt;

    CELLULAR_HAL_DBG_PRINT("%s: Context updated: iTotalNoofCellInfo=%u iTotalNoofIntraFreqCellInfo=%u\n", __FUNCTION__,
        nasCtx->iTotalNoofCellInfo, nasCtx->iTotalNoofIntraFreqCellInfo);

CLEANUP:
    if (pIntraFreqCells) g_array_free(pIntraFreqCells, TRUE);
...
static void cellular_hal_qmi_get_modem_vendor (QmiClientDms *client,
                                                   GAsyncResult *res,
                                                   gpointer  user_data)
{
    ...
    if( NULL != pstCtxGetIDs )
    {
        if( DMS_GET_MANUFACTURER_VIA_DEVICE_OPEN == pstCtxGetIDs->enQuerySource )
        {
            task  = (GTask *)pstCtxGetIDs->vpPrivateData;
            QMIContextStructPrivate    *pstQMIContext   = NULL;
        ...
        else if( DMS_GET_MANUFACTURER_VIA_DML == pstCtxGetIDs->enQuerySource )
        {
            dmsCtx = (ContextDMSInfo*)pstCtxGetIDs->vpPrivateData;
        }
        ...
        if (!output) {
            CELLULAR_HAL_DBG_PRINT("%s Failed to get modem vendor information: %s\n",__FUNCTION__,error->message);
            g_error_free (error);
            goto NEXTSTEP;
        }

        if (!qmi_message_dms_get_manufacturer_output_get_result (output, &error)) {
            CELLULAR_HAL_DBG_PRINT("%s Unable to get Modem Vendor: %s\n",__FUNCTION__,error->message);
            g_error_free (error);
        ...
        if( NULL !=  dmsCtx){
            snprintf(dmsCtx->MANUFACTURER, sizeof(dmsCtx->MANUFACTURER), "%s", str);
            CELLULAR_HAL_DBG_PRINT("%s MANUFACTURER retrieved: '%s'\n",__FUNCTION__,dmsCtx->MANUFACTURER);
	}
...
static void cellular_hal_qmi_get_modem_vendor_information( ContextGetIDs *pstCtxGetIDs )
{
    if( DMS_GET_MANUFACTURER_VIA_DEVICE_OPEN == pstCtxGetIDs->enQuerySource )
    {
        GTask        *task  = (GTask *)pstCtxGetIDs->vpPrivateData;
        ...
        qmi_client_dms_get_manufacturer (QMI_CLIENT_DMS(dmsCtx->dmsClient),
                                NULL,
                                10,
                                NULL,
                                (GAsyncReadyCallback)cellular_hal_qmi_get_modem_vendor,
                                pstCtxGetIDs);
    }
    else if( DMS_GET_MANUFACTURER_VIA_DML == pstCtxGetIDs->enQuerySource )
    {
...
static void cellular_hal_qmi_device_open_step( GTask *task )
{
    ...
    switch ( pDeviceOpenCtx->uiCurrentStep ) 
    {
        ...
        case MODEM_OPEN_STATE_GET_MANUFACTURER:
        {
            ContextGetIDs *pstCtxGetIDs = NULL;
            pstCtxGetIDs = (ContextGetIDs*)g_slice_new0 (ContextGetIDs);
            if( NULL != pstCtxGetIDs){
                pstCtxGetIDs->enQuerySource = DMS_GET_MANUFACTURER_VIA_DEVICE_OPEN;
                pstCtxGetIDs->IsQueryDone = FALSE;
                pstCtxGetIDs->vpPrivateData = (void*)task;

                cellular_hal_qmi_get_modem_vendor_information( pstCtxGetIDs );
            }
            return;
        }
...
int cellular_hal_qmi_get_manufacturer( char *manufacturer )
{
    //Check whether QMI ready or not
    if( ( NULL != gpstQMIContext ) && \
        ( NULL != gpstQMIContext->qmiDevice ) && \
        ( TRUE == qmi_device_is_open( gpstQMIContext->qmiDevice ) ) )
    {
        ...
        if( '\0' != dmsCtx->MANUFACTURER[0] )
        {
            memcpy( manufacturer, dmsCtx->MANUFACTURER, strlen(dmsCtx->MANUFACTURER) + 1 );
            CELLULAR_HAL_DBG_PRINT("%s - MANUFACTURER '%s' Retrived\n", __FUNCTION__,manufacturer);
        }
        else
        {
            ContextGetIDs *pstCtxGetIDs = NULL;
            ...
            if( NULL != pstCtxGetIDs )
            {
                pstCtxGetIDs->enQuerySource = DMS_GET_MANUFACTURER_VIA_DML;
                pstCtxGetIDs->IsQueryDone = FALSE;
                pstCtxGetIDs->vpPrivateData = (void*)(dmsCtx);
                cellular_hal_qmi_get_modem_vendor_information( pstCtxGetIDs );
                ...
                while ( 1 )
                {
                    ...
                    {
                        memcpy(  manufacturer, dmsCtx->MANUFACTURER, strlen(dmsCtx->MANUFACTURER) + 1 );
                        CELLULAR_HAL_DBG_PRINT("%s - Modem vendor '%s' retrived\n", __FUNCTION__,manufacturer);
                        break;
                    }
                    else if( iLapsedSeconds >= CELLULAR_QMI_GETIDS_VIA_DML_MAX_WAITIME )
                    {
                        CELLULAR_HAL_DBG_PRINT("%s - Timeout during IDs Query so sending empty  manufacturer\n",__FUNCTION__);
                        break;
                    }
    ...
    {
        CELLULAR_HAL_DBG_PRINT("%s - QMI(%s) not ready so can't get  modem vendor\n", __FUNCTION__, gpstQMIContext->modem_device_name);
        return RETURN_ERROR;
    }
}
                                                                        ...
                                                                        gpointer  user_data)
{
    ...
    if ( getProfileReset == 1)
    {
        getProfileReset = 0; //PDN list attached. Set getProfileReset to 0 to trigger Cellular Manager restart
        CELLULAR_HAL_DBG_PRINT ("%s Sending profile status as 'Created' since PDN list is attached for the default profile\n",__FUNCTION__);
    }
                                                                       ...
                                                                       gpointer  user_data)
{
    ...
    {
        //Printing Current PDN Profile List
        char logBuffer[32] = {0};
        ...
        for (i = 0; i < current_list->len; i++)
        {
            ...
            logptr += snprintf(logptr, sizeof(logBuffer) - (logptr - logBuffer), "%s%u",
                    i > 0 ? ", " : "", ui16PDNProfile);
        ...
        CELLULAR_HAL_DBG_PRINT ("\tCurrent list: '%s'\n", logBuffer);
    }
    ...
    {
        //Printing Pending PDN Profile List
        char logBuffer[32] = {0};
        char *logptr = logBuffer;
        ...
        CELLULAR_HAL_DBG_PRINT ("\tPending list: '%s'\n", logBuffer);
    }
...
static void cellular_hal_qmi_profile_operation_step( GTask *task )
{
    ...
    switch ( pProfileCrCtx->uiCurrentStep ) 
    {
        ...
        case WDS_PROFILE_MODIFY_VERIFY_PROFILE_PRESENT:
        {
            ...
            {
                CELLULAR_HAL_DBG_PRINT("%s %d - Verifying if WDS Profile exists before creation/modification\n", __FUNCTION__, __LINE__);
                CellularProfileStruct    *pstInputProfile   = &(pProfileCrCtx->stProfileInput);
                CellularProfileStruct    *pstProfileList    = wdsCtx->pstProfileList;

                if( NULL != wdsCtx->pstProfileList )
                {
                    CELLULAR_HAL_DBG_PRINT("%s %d - WDS Profile list is not empty\n", __FUNCTION__, __LINE__);
                    ...
                    if ( NULL != pstInputProfile )
                    {
                        ...
                        for( i = 0; i < wdsCtx->ui8ProfileCount; i++ )
                        {
                                ...
                                ( pstProfileList[i].PDPAuthentication == pstInputProfile->PDPAuthentication ) )
                            {
                                ...
                                getProfileReset = 1; //Indicate that the profile matches an existing one and is ready
                ...
                // Restart the Cellular Manager if the profile list is empty and fetching profiles fails after 20 retries; no extra profiles should be added.
                if (NULL == wdsCtx->pstProfileList)
                {
                    CELLULAR_HAL_DBG_PRINT("%s %d - WDS Profile list is empty\n", __FUNCTION__, __LINE__);
                    if (!pProfileCrCtx->bIsProfileAlreadyPresent && getProfileRetry > 20)
                    {
                        CELLULAR_HAL_DBG_PRINT("%s - Restarting the Cellular Manager as no profile was found even after 20 retries.\n", __FUNCTION__);
                        exit(0);
                    }
                }
                else if (!pProfileCrCtx->bIsProfileAlreadyPresent) // If the profile list is available but the profile is not already present
                {
                    // Reset the retry count and increment the retry counter for profile creation
                    getProfileRetry = 0;
                    getProfileRetryToCreate++;
                    CELLULAR_HAL_DBG_PRINT("%s - Profile list is available, but the profile is not present.\n", __FUNCTION__);

                    int i;
                    // Loop through the existing modem profile list to check if the input profile matches.
                    for (i = 0; i < wdsCtx->ui8ProfileCount; i++)
                    {
                         if ((strcasecmp(pstProfileList[i].APN, pstInputProfile->APN) == 0) &&
                             (strcmp(pstProfileList[i].Username, pstInputProfile->Username) == 0) &&
                             (strcmp(pstProfileList[i].Password, pstInputProfile->Password) == 0) &&
                             (pstProfileList[i].PDPType == pstInputProfile->PDPType) &&
                             (pstProfileList[i].PDPAuthentication == pstInputProfile->PDPAuthentication))
                         {
                             // If a match is found, mark the profile as already present and reset retry counters.
                             pProfileCrCtx->bIsProfileAlreadyPresent = TRUE;
                             getProfileReset = 1; //Indicate that the profile matches an existing one and is ready
                             getProfileRetryToCreate = 0;

                             CELLULAR_HAL_DBG_PRINT("%s - Matched with existing profile(%d). so ignore profile creation/modification\n",__FUNCTION__,pstProfileList[i].ProfileID);

                             if (pstInputProfile->bIsThisDefaultProfile)
                             {
                                 pstProfileList[i].bIsThisDefaultProfile = TRUE;
                             }
                     ...
                     // After 2 retries, reset the retry counter and proceed to profile creation
                     if (getProfileRetryToCreate > 2)
                     {
                         getProfileRetryToCreate = 0;
                         CELLULAR_HAL_DBG_PRINT("%s - Profile list is available, but the profile is not present. Creating the profile...\n", __FUNCTION__);
                     }
                }
        ...
        {
            // Proceed with profile creation if the operation is to create, either with no profile status callback or with a callback and after 2 retries.
            if ( WDS_PROFILE_OPERATION_CREATE == pProfileCrCtx->enProfileOperationRequest &&
                 getProfileRetryToCreate == 0 &&
                 getProfileRetry == 0)
            {
                CELLULAR_HAL_DBG_PRINT("%s %d - Entering WDS Profile Create\n", __FUNCTION__, __LINE__);
                CellularProfileStruct    *pstInputProfile   = &(pProfileCrCtx->stProfileInput);
        ...
        {
            // Proceed with profile creation/modification if the operation is to create/modify, either with no profile status callback or with a callback and after 2 retries.
            if ( ( WDS_PROFILE_OPERATION_CREATE == pProfileCrCtx->enProfileOperationRequest &&
                 getProfileRetryToCreate == 0 &&
                 getProfileRetry == 0 ) ||
                 ( WDS_PROFILE_OPERATION_MODIFY == pProfileCrCtx->enProfileOperationRequest ) )
            {
                CELLULAR_HAL_DBG_PRINT("%s %d - Entering WDS Profile Create/Modify to set default profile\n", __FUNCTION__, __LINE__);
                CellularProfileStruct    *pstInputProfile   = &(pProfileCrCtx->stProfileInput);
        ...
        case WDS_PROFILE_CREATE_VERIFY_AND_SET_LTE_PDN_ATTACH:
        {
            ...
            if ( ( WDS_PROFILE_OPERATION_CREATE == pProfileCrCtx->enProfileOperationRequest &&
                 ...
                 ( WDS_PROFILE_OPERATION_MODIFY == pProfileCrCtx->enProfileOperationRequest ) )
            {
                CELLULAR_HAL_DBG_PRINT("%s %d - Entering WDS Profile Create/Modify to verify and set LTE PDN attach\n", __FUNCTION__, __LINE__);
                CellularProfileStruct    *pstInputProfile   = &(pProfileCrCtx->stProfileInput);
        ...
        case WDS_PROFILE_MODIFY_NOTIFY:
        {
            if ( ( WDS_PROFILE_OPERATION_CREATE == pProfileCrCtx->enProfileOperationRequest ) || 
                 ...
                 ( WDS_PROFILE_OPERATION_MODIFY == pProfileCrCtx->enProfileOperationRequest ) )
            {
                CELLULAR_HAL_DBG_PRINT("%s %d - Entering WDS Profile Create/Delete/Modify notification\n", __FUNCTION__, __LINE__);
                CellularProfileStruct    *pstInputProfile   = &(pProfileCrCtx->stProfileInput);

                //Send Profile ready status if CB is not null
                if( NULL != pProfileCrCtx->device_profile_status_cb )
                {
                    // Send a "profile status not ready" callback if the profile retry or retry to create is active, indicating that the profile is not yet ready.
                    if( getProfileRetry || getProfileRetryToCreate)
                    {
                    ...
                    else
                    {
                        ...
                        // Send "profile status ready" if the profile is already available; otherwise, send "profile status created" for a newly created profile.
                        if ( getProfileReset == 1 )
                        {
                        ...
                        {
                            pProfileCrCtx->device_profile_status_cb( pstInputProfile->ProfileName, pstInputProfile->PDPType, DEVICE_PROFILE_STATUS_CREATED );
                        }
                    }
                }
...
int cellular_hal_qmi_profile_operation(QMIHALProfileOperationInputStruct *pstProfileOperationInput)
{
        ...
        ( TRUE == qmi_device_is_open( gpstQMIContext->qmiDevice ) ) )
    {
        ...
        {
            CELLULAR_HAL_DBG_PRINT("%s %d - Creating WDS Profile\n",__FUNCTION__,__LINE__);
            ...
            if(NULL != pstProfileCreateDeleteModify->pstProfileInput)
            {
                ...
                // Check if the APN is empty and configure the default profile
                if( pstProfileCrCtx->stProfileInput.APN[0] == '\0' )
                {
                ...
                {
                  CELLULAR_HAL_DBG_PRINT("%s - Default Profile APN is already configured and not empty\n", __FUNCTION__);
                }
            }
            else
            {
        ...
        {
            CELLULAR_HAL_DBG_PRINT("%s %d - Deleting WDS Profile\n",__FUNCTION__,__LINE__);
            QMIHALProfileCreateDeleteModifyStruct *pstProfileCreateDeleteModify = &(punProfileOperation->stProfileCreateDeleteModify);
        ...
        {
            CELLULAR_HAL_DBG_PRINT("%s %d - Modifying WDS Profile\n",__FUNCTION__,__LINE__);
            QMIHALProfileCreateDeleteModifyStruct *pstProfileCreateDeleteModify = &(punProfileOperation->stProfileCreateDeleteModify);
        ...
        {
            CELLULAR_HAL_DBG_PRINT("%s %d - Listing WDS Profile\n",__FUNCTION__,__LINE__);
...
static void
cellular_hal_qmi_get_current_data_bearer_technology_cb (QmiClientWds *client,
                                                        GAsyncResult *res,
                                                        gpointer  user_data)
{
    GError *error = NULL;
    ...
    {
        CELLULAR_HAL_DBG_PRINT("%s - error: couldn't get current data bearer tech %s\n", __FUNCTION__,error->message);
        g_error_free (error);
    ...
    if (network_type == QMI_WDS_NETWORK_TYPE_3GPP)
    {
         switch (rat_mask)
         {
              case QMI_WDS_RAT_3GPP_GPRS:
              case QMI_WDS_RAT_3GPP_EDGE:
                  strncpy(wdsCtx->currentRAT,"GSM",sizeof(wdsCtx->currentRAT));
                  break;

              case QMI_WDS_RAT_3GPP_WCDMA:
              case QMI_WDS_RAT_3GPP_HSDPA:
              case QMI_WDS_RAT_3GPP_HSUPA:
              case QMI_WDS_RAT_3GPP_HSDPAPLUS:
              case QMI_WDS_RAT_3GPP_DCHSDPAPLUS:
                  strncpy(wdsCtx->currentRAT,"UMTS",sizeof(wdsCtx->currentRAT));
                  break;

              case QMI_WDS_RAT_3GPP_LTE:
                  strncpy(wdsCtx->currentRAT,"LTE",sizeof(wdsCtx->currentRAT));
                  break;

              case QMI_WDS_RAT_3GPP_NULL_BEARER:
              case QMI_WDS_RAT_3GPP_NONE:
              default:
                  strncpy(wdsCtx->currentRAT,"UNKNOWN",sizeof(wdsCtx->currentRAT));
                  break;
        }
    }
    else if (network_type == QMI_WDS_NETWORK_TYPE_3GPP2)
    {
         switch (rat_mask)
         {
              case QMI_WDS_RAT_3GPP2_CDMA1X:
                  strncpy(wdsCtx->currentRAT,"CDMA20001X",sizeof(wdsCtx->currentRAT));
                  break;

              case QMI_WDS_RAT_3GPP2_EVDO_REVA:
              case QMI_WDS_RAT_3GPP2_EVDO_REV0:
              case QMI_WDS_RAT_3GPP2_EVDO_REVB:
                  strncpy(wdsCtx->currentRAT,"EVDO",sizeof(wdsCtx->currentRAT));
                  break;

              case QMI_WDS_RAT_3GPP2_NONE:
              case QMI_WDS_RAT_3GPP2_NULL_BEARER:
              default:
                  strncpy(wdsCtx->currentRAT,"UNKNOWN",sizeof(wdsCtx->currentRAT));
                  break;
        }
...
static void cellular_hal_qmi_start_network_connection_step( GTask *task )
{
	...
	{
             unsigned char bIsIPv4 = ( CELLULAR_NETWORK_IP_FAMILY_IPV4 == pNWStartCtx->ip_request_type ) ? TRUE : FALSE;
             qmi_client_wds_get_current_data_bearer_technology (QMI_CLIENT_WDS(( TRUE == bIsIPv4 ) ?
                                                                pstQMIContext->wdsCtx.client_ipv4 : pstQMIContext->wdsCtx.client_ipv6),
                                                                NULL,
                                                                10,
                                                                NULL,
                                                                (GAsyncReadyCallback)cellular_hal_qmi_get_current_data_bearer_technology_cb,
                                                                task);
              return;
	}
