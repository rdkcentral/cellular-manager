...
{   
    ...
    {
        gpstCellularPolicyCtrl->bModemMode = TRUE;
    }
    else
    {
        gpstCellularPolicyCtrl->bModemMode = FALSE;
    }

    UpdateSMControlStateEnable();
#endif /** !_SR213_PRODUCT_REQ_ */

    CcspTraceInfo(("%s - Received Device Open Status - DeviceName[%s] Status[%d] Wwan[%s] IsModemOnline[%d]\n", __FUNCTION__, device_name, device_open_status, wan_ifname, gpstCellularPolicyCtrl->bModemMode));
    if(device_open_status == DEVICE_OPEN_STATUS_READY){
    v_secure_system("echo 1500 > /sys/class/net/wwan0/qmi/rx_urb_size");
    CcspTraceInfo(("%s - Received Device Open Status - Set rx_urb_size=1500 completed \n", __FUNCTION__));
    }
...
{   
    ...
    if( CELLULAR_NETWORK_IP_FAMILY_IPV4 == pstIPStruct->IPType )
    {
       ...
       v_secure_system("echo %d > /sys/class/net/wwan0/qmi/hard_mtu",pstIPStruct->MTUSize);
       v_secure_system("ifconfig wwan0 %s netmask %s up", pstIPStruct->IPAddress,pstIPStruct->SubnetMask);
#endif
        if(sysevent_fd >= 0)
        {
...
#ifndef LTE_USB_FEATURE_ENABLED
       v_secure_system("echo %d > /sys/class/net/wwan0/qmi/hard_mtu",pstIPStruct->MTUSize);
       v_secure_system("ifconfig wwan0 %s up", pstIPStruct->IPAddress);
#endif
        if(sysevent_fd >= 0)
        {
...
void UpdateSMControlStateEnable( void )
{
    ...
    {
        CcspTraceError (("%s %d: pstDmlCellular is empty\n", __FUNCTION__, __LINE__));
        return;
    }

    if ( (pstDmlCellular->X_RDK_Enable == TRUE) && (gpstCellularPolicyCtrl->bModemMode == TRUE))
    {
          gpstCellularPolicyCtrl->bRDKEnable = TRUE;
    }
    else
    {
          gpstCellularPolicyCtrl->bRDKEnable = FALSE;
    }
    CcspTraceInfo(("%s - Received RDK Enable: [%d], Current Modem Mode: [%d], Updated RDK Enable: [%d]\n", __FUNCTION__, pstDmlCellular->X_RDK_Enable, gpstCellularPolicyCtrl->bModemMode, gpstCellularPolicyCtrl->bRDKEnable));
}
...
STATIC CellularPolicySmState_t StateDeregistered( void )
{
    ...
    if( DEVICE_NAS_STATUS_REGISTERED == gpstCellularPolicyCtrl->enDeviceNASRegisterStatus )
    {
        ...
        // After successful modem registration, initialize the context with the default profile
        // based on the provided JSON values. The initialization will be done using the MCCMNC values
        // to ensure the correct profile is selected based on the network operator.
        CellularCurrentPlmnInfoStruct stPlmnInfo = {0};
        CellularContextInitInputStruct stCtxInputStruct = { 0 };

        cellular_hal_get_current_plmn_information( &stPlmnInfo );
        if(stPlmnInfo.MCC != 0 && stPlmnInfo.MNC!=0)
        {
            snprintf( MCCMNC, sizeof(MCCMNC), "%d%d", stPlmnInfo.MCC, stPlmnInfo.MNC );
        }
        else
        {
            MCCMNC[0] = '\0';
        ...
        CellularMgr_InitializeContextDefaultProfile( &stCtxInputStruct ); // Initialize context profile based on MCCMNC values

        cellular_hal_init_ContextDefaultProfile( &stCtxInputStruct ); //Initialize context profile via QMI

        if( gpstCellularPolicyCtrl->stContextProfile.APN[0] == '\0' )
        {
            memcpy(&(gpstCellularPolicyCtrl->stContextProfile), &(stCtxInputStruct.stIfInput), sizeof(CellularProfileStruct));
        }
...
STATIC CellularPolicySmState_t StateRegistered( void )
{
    ...
    // Restart the Cellular after Default Profile is created and selected
    if(( DEVICE_NAS_STATUS_REGISTERED == gpstCellularPolicyCtrl->enDeviceNASRegisterStatus ) &&
       ( DEVICE_PROFILE_STATUS_CREATED == gpstCellularPolicyCtrl->enDeviceProfileSelectionStatus ) )
    {
        CcspTraceInfo(("%s:%d Restarting the Cellular due to Default Profile being successfully created.\n", __FUNCTION__, __LINE__));
        exit(0);
    }
