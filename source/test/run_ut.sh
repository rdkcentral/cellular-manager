...
log "INFO" "Running make for RdkCellularManager_gtest.bin..."
...
if [ -f "./source/test/RdkCellularManager_gtest.bin" ]; then
    log "Running RdkCellularManager_gtest.bin"
    ./source/test/RdkCellularManager_gtest.bin
    log "INFO" "Completed Test Execution"
else
    log "ERROR" "RdkCellularManager_gtest.bin does not exist, cannot run tests"
    exit 1
fi
